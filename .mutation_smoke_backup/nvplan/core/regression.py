...
Every fit is also graded "good" / "fair" / "review" on its final R^2 (C1, ``_grade_fit``,
``FitResult.quality`` / ``.quality_reason``, recorded in the derivation like the two guards
above). "review" is not a third guard that falls back to a safe number -- there is no safe
number for a split that fails it -- so :func:`nvplan.core.projector.project_scenario` refuses to
project a "review"-graded category at all. See that module's docstring for why an
investment-driven category never reaches this grading in the first place.
...
# --------------------------------------------------------------------------- fit-quality grading (C1)
#
# Every category NewVision fits is already revenue-driven by construction: depreciation (DEPR)
# is never regressed at all -- it is a straight-line schedule off the investment plan
# (nvplan.core.depreciation), computed independently of revenue. So the reference brief's
# selection ladder ("route by driver so an investment-driven category never gets fit against
# revenue, because a monotone asset run-off can score R^2 = 0.98 against a coincidentally
# declining revenue") is already structurally satisfied here: there is no path by which DEPR
# could be regressed against REV. What is still missing is grading the regression MAT/EXT/PERS/OTH
# actually get, and refusing to project a grade that says the split should not be trusted.
#
# This is a sibling to, not a replacement for, the two guards already here: the valorization
# guard (5.2) judges whether the *derived growth rate* of the fixed part is meaningful and falls
# back to 0.0 when it is not; the joint plausibility guard (5.4) judges whether a *converged*
# joint fit is economically sane and falls back to OLS when it is not. Both survive their own
# failure by falling back to a defensible number. This guard judges the split itself (its R^2)
# and there is no honest fallback for a split that fails it -- a level or a run-off is not
# available for costs the client's own model defines as revenue-driven -- so it does not fall
# back: nvplan.core.projector.project_scenario refuses to project a "review"-graded category at
# all (ValueError), rather than persisting a plan built on it.
GOOD_R2 = 0.80
"""At or above this R^2, the fixed/variable split is strong enough to project without comment."""

FAIR_R2 = 0.60
"""Below GOOD_R2 but at or above this, the split is usable but its explanatory power is only
moderate -- surfaced as a warning on the fit, not a refusal to project."""

FIT_QUALITIES: tuple[str, ...] = ("good", "fair", "review")


def _grade_fit(r_squared: float) -> tuple[str, str | None]:
    """Grade a fixed/variable split's R^2 into "good" / "fair" / "review".

    Returns ``(quality, reason)`` with ``reason is None`` iff ``quality == "good"``.
    """
    if r_squared >= GOOD_R2:
        return "good", None
    if r_squared >= FAIR_R2:
        return "fair", (
            f"R^2 of {r_squared:.0%} is below the {GOOD_R2:.0%} good threshold: the fixed/"
            "variable split explains this category's historical variation only moderately well."
        )
    return "review", (
        f"R^2 of {r_squared:.0%} is below the {FAIR_R2:.0%} fair threshold: the fixed/variable "
        "split does not describe this category's historical variation well enough to project "
        "unreviewed."
    )
...
class FitResult:
    ...
    #: "good" | "fair" | "review" (C1, see ``_grade_fit``): grades the fixed/variable split's
    #: R^2 -- a different question from ``valorization_status`` (is the derived growth rate
    #: meaningful) or ``joint_status`` (is a converged joint fit plausible). "review" means
    #: :func:`nvplan.core.projector.project_scenario` refuses to project this category.
    quality: str = "good"
    #: Human-readable reason, mirroring ``valorization_fallback_reason`` /
    #: ``joint_rejection_reason``. ``None`` exactly when ``quality == "good"``.
...
def fit_category(
    ...
    * ``"joint"`` (VERIFICATION.md 5.1): :func:`fit_joint` fits alpha, v and beta together by
    ...
    # C1: grade the split actually used (post any joint acceptance/rejection above) on its R^2.
    quality, quality_reason = _grade_fit(r2)
